...
export default function ProductCard({
  ...
  // Per-product image scale
  const getImageScale = () => {
    const name = product.name.toLowerCase()
    if (name.includes('hldy zip')) return 0.95
    if (name.includes('melrose') || name.includes('swinger jean')) return 0.95
    if (name.includes('coyote bag')) return 0.9
    return 1.1
  }
  ...
  return (
    <div className='w-full select-none overflow-hidden bg-black rounded-sm'>
      ...
      {/* Image Area */}
      <div className='relative aspect-square md:aspect-[4/3] bg-[#1a1a1a]'>
        {/* Sold Out Pill */}
        {isSoldOut && (
        ...
        <div className='relative px-2 py-2'>
          {isSoldOut ? (
            ...
            <div className='flex flex-wrap justify-center gap-1'>
              {allVariants.length === 1 && ['default title', 'one size', 'os'].includes(allVariants[0].size.toLowerCase()) ? (
                <button
